...
class TPoint {
 public:
    ...
    // rotation
    void rotate90_clockwise() { swap(x, y); y *= -1; }
    void rotate90_counterclockwise() { swap(x, y); x *= -1; }
    template<typename T> void rotate(const T &radians) {
        Type new_x = x * cos(radians) - y * sin(radians);
        Type new_y = x * sin(radians) + y * cos(radians);
        x = new_x; y = new_y;
    }
...
template<typename Type, typename T> TPoint<Type> rotated(const TPoint<Type> &p, const T &radians) {
    return TPoint<Type>(p.x * cos(radians) - p.y * sin(radians), p.x * sin(radians) + p.y * cos(radians));
}
template<typename T> std::string to_string(const TPoint<T> &p) {
    return "(" + to_string(p.x) + ", " + to_string(p.y) + ")";
}

// choose type you need
